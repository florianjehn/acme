...
def convert_fluxes_for_fluxogram(all_fluxes):
    ...
    # to another node. If you go through all nodes, this results in 
    # redundant data
    for from_storage in all_fluxes:
        for to_storage in all_fluxes[from_storage]:
            ...
            # negative value that needs to be kicked out for further analsys
            if all_fluxes[from_storage][to_storage] >= 0 or (
                                all_fluxes[from_storage][to_storage] != -0.0):
...
if __name__ == "__main__":
    ...
    # through it, nodes with now fluxes and storage can occur
